...
export const TOOL_EXTENDED_CONTENT_B4: Record<string, ToolExtendedContent> = {
    ...
    tabnine: {
        overviewHtml: `
            <p><strong>Tabnine</strong> predates the current generation of AI coding assistants and has spent the years since being out-featured by them. It is still deployed widely, and the reason has almost nothing to do with completion quality. Tabnine competes on where the software runs and on what its vendor can prove about the model's training data. For the organizations that buy it, those are not nice-to-haves layered on top of a coding tool; they are the entire purchasing criterion, and everything else is a secondary score. Tabnine has now aligned its pricing with that reality: there is no free tier and no individual plan left, and the published plans are annual, per-seat, and sold to organizations.</p>
            ...
            <p>The second trade is felt before you ever install anything, and it has hardened. There is no casual evaluation path left at all. Tabnine's published plans are a Code Assistant Platform at $39/user/month and an Agentic Platform at $59/user/month, both on annual subscriptions, with the pricing page built around requesting a quote; the free tier and the individual paid tier it used to sell are gone. So the old complaint — that a developer trying the entry tier was testing a product stripped of its thesis and concluding, reasonably, that it was unremarkable — has been replaced by a sharper one. You now cannot sample the product at all without a purchasing conversation and a year's commitment, and the deployment options that are the entire reason to choose Tabnine are quoted per customer rather than listed on a checkout page. That makes the decision a procurement exercise from the first minute. <a href="/compare/cursor-vs-tabnine">Comparing it with Cursor</a> on completions alone reaches a verdict against Tabnine and misses the point in exactly the way it always did — except that now you would have to sign an annual contract to run the comparison honestly.</p>
