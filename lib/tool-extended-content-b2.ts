...
export const TOOL_EXTENDED_CONTENT_B2: Record<string, ToolExtendedContent> = {
    ...
    "amazon-q-developer": {
        overviewHtml: `
            ...
            <p>Outside AWS-shaped work, Q Developer is competent rather than exciting. Inline suggestions, chat, test generation, documentation, and an agentic mode that takes a task and edits multiple files all work, and developers who have used both consistently rate its everyday fluency below Cursor and <a href="/tool/windsurf-ide">Windsurf</a>. The free individual tier — usable with a Builder ID, without an AWS account attached — means this does not have to be an either/or. Plenty of people keep it installed for the AWS half of the job and drive something else for the rest, which is a perfectly sane arrangement and arguably the intended one. If that broader comparison is what you are actually resolving, <a href="/compare/cursor-vs-amazon-q-developer">Cursor versus Amazon Q Developer</a> takes it head-on, and <a href="/blog/cursor-vs-github-copilot">Cursor versus Copilot</a> covers the assistant you will probably pair it with.</p>
